...
if [[ ! -d "$ANYKERNEL3_VERSION" ]]; then
	...
	# Clone only Spacewar version
	git clone --depth=1 https://github.com/William24hmar/AnyKernel3 -b master "${ANYKERNEL3_VERSION}"
else
...
# User details
KBUILD_USER="Willay24"
KBUILD_HOST="Kali_GNU/Linux-2025.2"
#KBUILD_USER="$USER"
#KBUILD_HOST=$(uname -n)
...
config_generator()
{
	...
	if [[ ! -f arch/arm64/configs/$DFCF ]]; then
		...
		muke "$DFCF" vendor/lahaina_QGKI.config vendor/debugfs.config savedefconfig
