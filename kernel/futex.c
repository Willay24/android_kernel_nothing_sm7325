...
/* Four entries cover the common wait set without growing the syscall frame. */
#define FUTEXV_STACK_MAX	4
 ...
 * same event, and if more than one futex was woken, the returned index may
 * refer to any one of them. (It is not necessarily the futex with the
		...
		struct __kernel_timespec __user *, timeout, clockid_t, clockid)
{
	...
	if (timeout) {
		...
		/*
		 * Since there's no opcode for futex_waitv, use FUTEX_WAIT_BITSET
		 * that uses absolute timeout as well. For a monotonic clock the
