...
int send_tfa_cal_apr(void *buf, int cmd_size, bool bRead)
{
	...
	if (0 == tfa_cal->map_data.dma_buf ) {
		...
		pr_err("%s: paddr 0x%pK, kvaddr 0x%pK, map_size 0x%x\n",
				__func__,
				&tfa_cal->cal_data.paddr,
				tfa_cal->cal_data.kvaddr,
				tfa_cal->map_data.map_size);
	...
	if (!bRead) {
		...
		/*Mark first 3K share memory dirty*/
		param_hdr.param_size = SZ_1K + SZ_2K;
	}
	...
	pr_err("%s: Sending tfa_cal port = 0x%x, cal size = %zd, cal addr = 0x%pK\n",
