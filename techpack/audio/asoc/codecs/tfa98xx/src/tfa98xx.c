...
static enum tfa_error tfa98xx_write_re25(struct tfa_device *tfa, int value)
{
	...
	if (err == tfa_error_ok) {
		err = tfa_dev_mtp_set(tfa, TFA_MTP_RE25_PRIM, value); /* set RE25 in shadow regiser */
		if (err == tfa_error_ok)
			err = tfa_dev_mtp_set(tfa, TFA_MTP_EX, 2);	 /* set MTPEX to copy RE25 into MTP  */
	}
...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if ((err == tfa_error_ok) && (tfa98xx->set_mtp_cal)) {
		enum tfa_error err_cal = tfa98xx_write_re25(tfa98xx->tfa, tfa98xx->cal_data);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	return send_tfa_cal_set_bypass(ptr, 4);
}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	return send_tfa_cal_set_tx_enable(ptr, 4);
}
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!tfa98xx->tapdet_enabled && enable)		
		tfa_irq_clear(tfa98xx->tfa, tfa9912_irq_sttapdet); /* clear pending event if any */
...
#ifndef CONFIG_NOTHING_SPACEWAR
/* Firmware management */
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	/* Preload settings using internal clock on TFA2 */
	if ((tfa98xx->tfa->tfa_family == 2) && (tfa98xx->tfa->is_probus_device == 0)) {
		mutex_lock(&tfa98xx->dsp_lock);
...
{
	unsigned int tap_pattern = tfa_get_tap_pattern(tfa98xx->tfa); 
	int btn = 0;
...
{
	struct tfa98xx *tfa98xx = container_of(work, struct tfa98xx, tapdet_work.work);
...
{
#ifndef CONFIG_NOTHING_SPACEWAR
	struct tfa98xx *tfa98xx = container_of(work, struct tfa98xx, nmodeupdate_work.work);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		if (ret == Tfa98xx_Error_Not_Supported) {
			...
			dev_dbg(&tfa98xx->i2c->dev,	"tfa_dev_start success (%d)\n",	tfa98xx->init_count);
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	if (tfa98xx->flags & TFA98XX_FLAG_TAPDET_AVAILABLE) {
		...
		if (tfa_irq_get(tfa98xx->tfa, tfa9912_irq_sttapdet)) {
			tfa98xx_tapdet(tfa98xx);
			tfa_irq_clear(tfa98xx->tfa, tfa9912_irq_sttapdet); /* clear interrupt */
		}
	...
	struct snd_soc_dai *dai)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (strncmp(basename, tfa_cont_profile_name(tfa98xx, prof), len) == 0) {
	...
	if (mute) {
		...
		if ((strcmp(tfa_cont_profile_name(tfa98xx, tfa98xx_mixer_profile), "handset") != 0)
			&& !(strstr(tfaContProfileName(tfa98xx->tfa->cnt,
					tfa98xx->tfa->dev_idx, tfa98xx_mixer_profile), ".standby") != NULL)) {
			tfa98xx_send_mute_cmd(TFA_KCONTROL_VALUE_ENABLED);
			msleep(60);
		}
#endif
		tfa98xx_sync_count = 0;
#if defined(CONFIG_NOTHING_SPACEWAR)
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
					      ...
					      unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_CMD_GET_MEMTRACK_DATA: {
		list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			if (tfa98xx->tfa->is_probus_device) {
				...
				/* primary */
				/* Re0: should be dividing 65536 in userspace */
				livedata[0] =
					(data[3] << 16) + (data[4] << 8) + data[5];
				/* Temperature: should be dividing 16384 in userspace */
				livedata[1] =
				...
				/* Re0: should be dividing 65536 in userspace */
				livedata[3] =
					(data[6] << 16) + (data[7] << 8) + data[8]; 
				/* Temperature: should be dividing 16384 in userspace */
				livedata[4] =
					((data[12] << 16) + (data[13] << 8) + data[14]);
				/* F0 */
				livedata[5] =
					(data[18] << 16) + (data[19] << 8) + data[20];
	...
	case IOCTL_CMD_GET_CNT_VERSION: {
		...
		result = copy_to_user((void __user *)pUserData,
				      tfa98xx_container->type, strlen(tfa98xx_container->type));
		if (!result)
